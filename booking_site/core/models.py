...
class RoomCategory(TimeStampedModel):
    ...
    price_per_night = models.DecimalField(max_digits=8, decimal_places=2, verbose_name="Ціні за ніч")
    ...
    def __str__(self):
        return f"{self.name} ({self.capacity} oc.)"
    
class Room(models.Model):
    number = models.IntegerField(unique=True, verbose_name="Номер кімнати")
    floor = models.PositiveSmallIntegerField(default=3, verbose_name="Поверх")
...
class Booking(TimeStampedModel):
    ...
    class Meta:
        ...
        ordering = ["-created_at"]
        constraints = [models.CheckConstraint(check=Q(check_out__gt=F("check_in")), name="check_out_gt_check_in")]
        indexes = [
    ...
    def clean(self):
        ...
        if self.check_in and self.check_out and self.check_out <= self.check_in:
            raise ValidationError("Дата виїзду має буди пізніше дати заїзду.")
            
        if self.room and self.check_in and self.check_out:
            qs = Booking.objects.filter(room = self.room, status__in = [BookingStatus.PENDING, BookingStatus.CONFIRMED], check_in__lt = self.check_out, check_out__gt = self.check_in)
            if self.pk:
                qs = qs.exclude(pk=self.pk)
            if qs.exists():
                raise ValidationError("Обраний період уже зайнятий для цієй кімнати.")
